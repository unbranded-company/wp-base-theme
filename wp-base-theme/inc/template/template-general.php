...
function wp_base_theme_scripts() {
    $styles = get_compiled_style();
    wp_enqueue_style('wp-base-theme-style', get_stylesheet_uri());
    foreach ($styles['css'] as $src) {
        wp_enqueue_style('wp-base-theme-' . $src['basename'], $src['path'], false, '1.0', 'all');
    }
    foreach ($styles['js'] as $src) {
        wp_enqueue_script('wp-base-theme-'. $src['basename'], $src['path'], array('jquery'), '1.0.0', true);
    }
